...
class SUtil
{
	...
	public static function doTheCheck()
	{
		...
		if (Permissions.getGrantedPermissions().contains(PermissionsList.READ_EXTERNAL_STORAGE) || Permissions.getGrantedPermissions().contains(PermissionsList.WRITE_EXTERNAL_STORAGE))
		{
			...
			{
				SUtil.applicationAlert('Oh Oh Error :(!', "storage/emulated/0/.ModsEngine/There are no 'assets' and 'mods', Extract files 'assets' and 'mods' from the app to '.ModsEngine'");
				System.exit(0);
			}
			else
			{
				...
				{
					SUtil.applicationAlert('Oh Oh Error :(!', "storage/emulated/0/.ModsEngine/There is no 'assets', Extract file 'assets' from the app to '.ModsEngine'");
					System.exit(0);
				}

				if (!FileSystem.exists(SUtil.getPath() + 'mods'))
				{
					SUtil.applicationAlert('Oh Oh Error :(!', "storage/emulated/0/.ModsEngine/There is no 'mods', Extract file 'mods' from the app to '.ModsEngine'");
					System.exit(0);
				}
			}
		}
	...
	public static function onCrash(e:UncaughtErrorEvent):Void
	{
		...
		var path:String = "crash/" + "crashEngine_" + dateNow + ".txt";
		var errMsg:String = "";
		...
		SUtil.applicationAlert("Oh Oh Error :(!", errMsg);
		System.exit(0);
	}
